...
client.on('messageCreate', async (message) => {
    ...
    if(!message.content.includes('"')){
        let text = message.content.substring(message.content.indexOf('>') + 1).trim();
        ...
        try {
            ...
            message.channel.send("Is the AE database down again? Tesla, I need your help.");
            }
        }
    }
});
...
async function searchFandom(searchTerm) {
    try {
        const url = `https://honkai-impact-3rd-archives.fandom.com/wiki/Special:Search?query=${encodeURIComponent(searchTerm)}`;
...
function customizeResponse(response, characterName) {
    ...
    response = response.replace(/I don't know/i, "I'm uncertain, Captain. The mysteries of the universe elude me.");
    ...
    if (response.length > maxLength) {
        ...
        response = lastPeriodIndex !== -1 ? response.slice(0, lastPeriodIndex + 1) : response.slice(0, maxLength);
